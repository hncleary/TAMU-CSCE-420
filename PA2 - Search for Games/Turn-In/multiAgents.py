...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        score, moveDecision = self.miniMax(gameState, 0, 0)
        return moveDecision
    ...
    def gameEnd(self, gameState, depth):
        win = gameState.isWin()
        lose = gameState.isLose()
        depthEnd = depth == self.depth
        if win or lose or depthEnd:
...
def betterEvaluationFunction(currentGameState):
    ...
    """
    # use current state --> rather than successor state
    # like reflex agent but current
    # successorGameState = currentGameState.generatePacmanSuccessor(action)
    ...
    totalGhostDistances = sum(ghostDistances)
    scared_heur = 0
    if totalScared > 0:
        scared_heur = 50*totalScared + 50*(-capsule_heur) + 100*(-totalGhostDistances)
    ...
    # calculate final heuristic
    final_heur = -food_heur -ghost_heur -food_left_heur + capsule_heur + scared_heur
